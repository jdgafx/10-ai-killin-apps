...
export async function onRequest(context: { request: Request; env: Env }) {
  ...
  try {
    ...
    const systemPrompt = `You are an autonomous research agent. Analyze queries and provide comprehensive research findings.

Research depth: ${body.depth || "quick"}

Provide structured research with:
1. **Key Findings**: Main discoveries and insights
2. **Sources**: Relevant references and citations
