...
export async function onRequest(context: { request: Request; env: Env }) {
  ...
  try {
    ...
    const taskPrompts: Record<string, string> = {
      ...
      extract:
        "Extract key information including: entities, dates, numbers, important facts, and actionable items.",
      analyze:
        "Provide a detailed analysis including: main themes, sentiment, structure, and recommendations.",
      translate: `Translate this document to ${body.language || "Spanish"} while preserving formatting and meaning.`,
    };

    const systemPrompt = `You are an expert document processor. ${taskPrompts[task]}
...
- Professional in tone`;
    ...
    const documentContent = body.document || body.message || "";
